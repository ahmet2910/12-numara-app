...
// WhatsApp tarzı **kalın** yazıları algılayan fonksiyon
const metniCozumle = (metin: string) => {
  ...
  return parcalar.map((parca, index) => {
    if (index % 2 === 1) {
      return <Text key={index} style={{ fontWeight: 'bold', color: 'white' }}>{parca}</Text>;
    }
    // Geri kalanlar normal yazılır
    return <Text key={index}>{parca}</Text>;
  });
};

export default function BasketbolHaberDetay({ route, navigation }: any) {
    ...
    <View style={styles.container}>
      ...
      <ScrollView bounces={false}>
        ...
        <View style={styles.contentContainer}>
          ...
          <Text style={styles.icerik}>{metniCozumle(haber.icerik)}</Text>
...
const styles = StyleSheet.create({
  ...
  icerik: { color: 'rgba(255,255,255,0.8)', fontSize: 16, lineHeight: 24, textAlign: 'justify'},
